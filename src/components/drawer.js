...
import { People, Map, Tv } from "@mui/icons-material";
import { useNavigate } from "react-router-dom";

const Drawer = ({ isOpen, drawerOptions, handleClose }) => {
          ...
          {drawerOptions.map((drawerOption) => (
            <ListItem key={drawerOption.text}>
              <ListItemButton onClick={() => setCurrentPath(drawerOption.path)}>
                <ListItemIcon>{drawerOption.icon}</ListItemIcon>
  ...
  return (
    <DrawerMaterial open={isOpen} onClose={handleClose}>
      {renderList}
    </DrawerMaterial>
  );
};
